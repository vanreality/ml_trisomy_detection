...
def extract_reads_from_bam(bam, txt, threshold, output):
    ...
    for label in ['0', '1']:
        ...
        extracted_reads = [read for read in bam.fetch() if read.query_name in read_names]
        ...
        output = pysam.AlignmentFile(out_name, "wb", template=bam)
...
@click.option('--output', required=True, type=click.Path(), help='Output file prefix')
